...
`interface` 常用于声明一个对象形式自定义类型，我们还可以使用 interface 来声明一个可以`约束函数属性和类` 的自定义类型
...
通常，我们谈起 ts 时，必须了解一个特点，就是 ts 类型只在编译(`ts -> js`)时影响源码，但就是有这样的一个 API，它参与了 js 运行时，就是这个 enum
...
记住 enum 开发尽量不要使用，毕竟会参与运行时，也可以对象来替代
